...
goToCreate(): void {
  ...
  if (this.hasGlPostedRows) {
    Swal.fire({
      icon: 'warning',
      title: 'GL Posted',
      text: 'Sales Invoice already GL posted. You cannot create new invoice.'
    });
    return;
  }
  ...
  edit(id: number): void {
    ...
    if (row && this.isRowLocked(row)) {
      Swal.fire({
        icon: 'warning',
        title: 'GST Locked',
        text: 'This invoice belongs to locked GST period.'
      });
      return;
    }
      ...
      this.si.delete(id).subscribe({
        ...
        error: (err) => {
          Swal.fire({
            icon: 'error',
            title: 'Delete failed',
            text: err?.error?.message || 'Something went wrong while deleting the invoice.'
          });
        }
      });
    });
  ...
  openLinesModal(row: SiListRow): void {
    this.activeSi = row;
    ...
    this.si.get(row.id).subscribe({
      next: (res: ApiResponse) => {
        ...
        this.modalGrand = lines.reduce(
          (sum: number, l: SiLine) => sum + this.lineTotal(l),
          0
        );
      },
      error: () => {
  ...
  sourceLabel(st: 1 | 2): string {
    return st === 1 ? 'SO' : 'DO';
  }

  printInvoice(row: SiListRow): void {
    this.si.get(row.id).subscribe({
      next: (res: ApiResponse) => {
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
    ...
    .metaCard {
      width:100%; margin:10px auto 14px auto;
      border:1px solid #d1d5db; border-radius:12px;
      background:#fff; padding:12px 14px;
      box-shadow:0 2px 10px rgba(0,0,0,.06);
    }
    .metaTitle {
      font-size:12px; font-weight:900; color:${brand};
    ...
    .tbl thead th {
      ...
      letter-spacing:.3px; border-color:${brand}!important;
    }
    .c { text-align:center; }
    .r { text-align:right; }
    .b { font-weight:900; }
...
<body>
  ...
  openEmailConfirm(row: any): void {
    this.emailConfirmRow = row;
    this.showEmailConfirmModal = true;
  }

  closeEmailConfirmModal(force = false): void {
    if (this.emailSending && !force) {
      return;
    }
  ...
  async sendSiEmail(row: any): Promise<void> {
    try {
  ...
  private async ensurePdfMakeReady(): Promise<any> {
    if (this._pdfReady && this._pdfMake) {
  ...
  return (this.rows || []).some(x => x.glPosted === true);
}
}